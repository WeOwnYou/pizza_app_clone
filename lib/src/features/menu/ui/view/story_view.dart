...
class StoryView extends StatefulWidget with AutoRouteWrapper {
  ...
  Widget wrappedRoute(BuildContext context) {
    return BlocProvider(
      create: (_) =>
          StoryBloc(context.read<IDodoCloneRepository>(), _initialStoryIndex),
      child: this,
      lazy: false,
    );
  }
  ...
  void pageControllerListener() {
    if ((_pageController.page ?? 0) % 1 != 0) {
      ...
      if (_pageController.page ==
          context.read<StoryBloc>().state.storyActiveIndex) {
                ...
                child: ColoredBox(
                  ...
                  child: BlocBuilder<StoryBloc, StoryState>(
                    buildWhen: (prevState, curState) {
                      if (prevState.status != curState.status &&
                          curState.status == StoryStateStatus.success) {
                        _animationController
                          ..duration = curState
                              .stories[widget._initialStoryIndex]
                              .storyItems
                              .first
                              .duration
                          ..forward();
                    ...
                    builder: (context, state) {
                      if (state.status == StoryStateStatus.failure) {
                        return const _BuildFailureWidget();
                      }
                      if (state.status != StoryStateStatus.success) {
                        return ColoredBox(
                          color: Colors.black,
                          child: Center(
                      ...
                      return GestureDetector(
                        ...
                        onVerticalDragEnd: (details) {
                          if (top > MediaQuery.of(context).size.height * 0.2) {
                            context.read<StoryBloc>().add(StoryCloseEvent());
                          } else {
                            _animationController.forward();
                            setState(() {
                              ...
                              .map(
                                (e) => _BuildStoryWidget(
                                  ...
                                  onTapDown: (details) {
                                    ...
                                    final forward =
                                        details.globalPosition.dx > centerWidth;
                                    _onTapDown(forward);
                                  },
                                  progressValue: state.storyActiveIndex == e.key
              ...
              children: [
                ...List.generate(
                  ...
                  (index) {
                    ...
                    if (index > state.storyItemsActiveIndexes[storyIndex]) {
                      currentValue = 0;
                    } else if (index <
        ...
        child: BlocBuilder<StoryBloc, StoryState>(
          builder: (context, state) {
            return ProductImageWidget(
              url: state.currentStoryItem.link,
              fit: BoxFit.fill,
            );
          },
        ),
